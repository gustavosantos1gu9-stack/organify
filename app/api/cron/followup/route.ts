...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // 1. Buscar follow-ups pendentes cujo horário já passou
    const { data: pendentes } = await supabase.from("followup_fila")
      .select("id")
      .eq("status", "pendente")
    ...
    for (const item of pendentes) {
      try {
        const res = await fetch(`${APP_URL}/api/ai/followup/enviar`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ followup_fila_id: item.id }),
        });
    ...
    console.log(`[cron/followup] ${enviados} enviados, ${erros} erros de ${pendentes.length} pendentes`);
    return NextResponse.json({ ok: true, processados: pendentes.length, enviados, erros });
  } catch (e: any) {
    console.error("[cron/followup] Erro:", e);
