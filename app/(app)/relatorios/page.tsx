...
import {
  ...
  FileText, Clock, Check, X, RefreshCw, ChevronDown, MessageCircle,
} from "lucide-react";
...
export default function RelatoriosPage() {
  ...
  // Token Meta (da agência)
  const [agToken, setAgToken] = useState("");

  useEffect(() => {
    carregarRelatorios();
    carregarTokenAgencia();
  }, []);

  async function carregarTokenAgencia() {
    const agId = await getAgenciaId();
    const { data } = await supabase.from("agencias").select("meta_business_token").eq("id", agId!).single();
    if (data?.meta_business_token) setAgToken(data.meta_business_token);
  }
  ...
  async function carregarContas() {
    const token = metaToken || agToken;
    if (!token) { alert("Configure o token Meta primeiro"); return; }
    ...
    try {
      ...
      else alert(data.error || "Erro ao carregar contas");
    } catch { alert("Erro ao conectar na Meta API"); }
  ...
  async function carregarGrupos() {
    ...
    try {
      const res = await fetch("/api/evolution", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "fetchChats" }),
      });
      const data = await res.json();
  ...
  async function salvar() {
    if (!nome.trim() || !nomeCliente.trim() || !adAccountId.trim()) {
      alert("Preencha nome, cliente e conta de anúncio");
      return;
    }
    if (destinoTipo === "grupo" && !grupoId) {
      alert("Selecione um grupo do WhatsApp");
      return;
    }
    if (destinoTipo === "contato" && !contatoNumero) {
      alert("Informe o número do contato");
      return;
    }
  ...
  async function verPreview() {
    ...
    // Salvar temporariamente pra gerar preview
    const agId = await getAgenciaId();
    ...
    if (!relId) { setPreviewLoading(false); alert("Erro ao gerar preview"); return; }
    ...
    try {
      ...
      const data = await res.json();
      setPreviewMsg(data.mensagem || data.error || "Erro ao gerar preview");
    } catch {
      setPreviewMsg("Erro ao conectar com a API");
    }

    // Limpar temp se não estava editando
    if (!editando) {
      await supabase.from("relatorios").delete().eq("id", relId);
    }
  ...
  async function duplicar(rel: Relatorio) {
    ...
    await supabase.from("relatorios").insert({
      agencia_id: agId,
      nome: `${rel.nome} (cópia)`,
      nome_cliente: rel.nome_cliente,
      ad_account_id: rel.ad_account_id,
      meta_token: rel.meta_token,
      grupo_id: rel.grupo_id,
      grupo_nome: rel.grupo_nome,
      contato_numero: rel.contato_numero,
      template: rel.template,
      periodo: rel.periodo,
      frequencia: rel.frequencia,
      horario_envio: rel.horario_envio,
      dia_semana: rel.dia_semana,
      ativo: false,
    });
    carregarRelatorios();
  }
  ...
  return (
    <div className="animate-in">
      <div className="breadcrumb">
        ...
        <span className="current">Relatórios</span>
      </div>

      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
        <h1 style={{ fontSize: "22px", fontWeight: "600" }}>Relatórios</h1>
        <button className="btn-primary" onClick={() => abrirModal()} style={{ cursor: "pointer", display: "flex", alignItems: "center", gap: "6px" }}>
      ...
      <div style={{ display: "flex", gap: "12px", marginBottom: "20px", flexWrap: "wrap", alignItems: "center" }}>
        <div style={{ position: "relative", flex: 1, minWidth: "200px" }}>
          ...
          <input className="form-input" placeholder="Pesquisar..." value={busca} onChange={e => setBusca(e.target.value)}
                ...
                <tr key={rel.id} style={{ borderBottom: "1px solid #1a1a1a" }}>
                  <td style={{ padding: "10px 14px" }}>
                    <div style={{
                      ...
                      background: rel.ativo ? "#22c55e" : "#ef4444",
                    }} />
                  </td>
                  <td style={{ padding: "10px 14px", color: "#606060", fontSize: "12px" }}>
                    {fmtData(rel.created_at)}
                  </td>
                  ...
                  <td style={{ padding: "10px 14px", color: "#a0a0a0", fontSize: "12px" }}>
                    {rel.grupo_nome || rel.contato_numero || "—"}
                  </td>
                  <td style={{ padding: "10px 14px" }}>
                    <span style={{
                      ...
                      {FREQUENCIAS.find(f => f.value === rel.frequencia)?.label || rel.frequencia}
                    </span>
                  </td>
                  <td style={{ padding: "10px 14px", color: "#a0a0a0", fontSize: "12px" }}>
                    {PERIODOS.find(p => p.value === rel.periodo)?.label || rel.periodo}
                  </td>
                  <td style={{ padding: "10px 14px", color: "#606060", fontSize: "12px" }}>
                    {fmtData(rel.ultimo_envio)}
                  </td>
                  <td style={{ padding: "10px 14px" }}>
                    <div style={{ display: "flex", gap: "4px" }}>
                      <button onClick={() => enviarAgora(rel)} disabled={enviando === rel.id}
                        title="Enviar agora"
          ...
          <div style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
              <h2 style={{ fontSize: "18px", fontWeight: "600" }}>
                {editando ? "Editar Relatório" : "Criar Relatório"}
              </h2>
              <button onClick={() => setModal(false)} style={{ background: "none", border: "none", cursor: "pointer", color: "#606060" }}>
                <X size={20} />
              </button>
            </div>
            ...
            {/* Token Meta (opcional - override) */}
            <div className="form-group" style={{ marginBottom: "16px" }}>
              <label className="form-label">Token Meta (opcional — usa o da agência se vazio)</label>
              <input className="form-input" placeholder="EAAxxxxxxx..." type="password" value={metaToken} onChange={e => setMetaToken(e.target.value)} />
            </div>
            ...
            <div className="form-group" style={{ marginBottom: "20px" }}>
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                <input className="form-input" placeholder="act_1234567890" value={adAccountId} onChange={e => setAdAccountId(e.target.value)} style={{ flex: 1 }} />
                <button className="btn-secondary" onClick={carregarContas} disabled={loadingContas} style={{ cursor: "pointer", whiteSpace: "nowrap" }}>
                  <RefreshCw size={13} /> {loadingContas ? "Carregando..." : "Buscar contas"}
                </button>
              </div>
              {contas.length > 0 && (
                <div style={{ marginTop: "8px", display: "flex", flexDirection: "column", gap: "4px", maxHeight: "150px", overflowY: "auto" }}>
                  {contas.map(c => (
                    <button key={c.id} onClick={() => { setAdAccountId(c.id); setContas([]); }}
                      style={{
                        display: "flex", justifyContent: "space-between", alignItems: "center",
                        padding: "8px 12px", background: adAccountId === c.id ? "rgba(41,171,226,0.1)" : "#1a1a1a",
                        border: `1px solid ${adAccountId === c.id ? "#29ABE2" : "#2e2e2e"}`,
                        borderRadius: "6px", cursor: "pointer", textAlign: "left", color: "#f0f0f0", fontSize: "12px",
                      }}>
                      <span>{c.name}</span>
                      <span style={{ color: "#606060" }}>{c.id}</span>
                    </button>
                  ))}
                </div>
              )}
            ...
            <div style={{ marginBottom: "20px" }}>
              ...
              <div style={{ display: "flex", gap: "8px", marginBottom: "8px" }}>
                <button onClick={() => setDestinoTipo("grupo")}
                  style={{
                    padding: "6px 16px", borderRadius: "6px", fontSize: "12px", cursor: "pointer",
                    background: destinoTipo === "grupo" ? "rgba(41,171,226,0.15)" : "#1a1a1a",
                    border: `1px solid ${destinoTipo === "grupo" ? "#29ABE2" : "#2e2e2e"}`,
                    color: destinoTipo === "grupo" ? "#29ABE2" : "#a0a0a0",
                  }}>
                  <MessageCircle size={12} style={{ marginRight: "4px", verticalAlign: "middle" }} /> Grupo
                </button>
                <button onClick={() => setDestinoTipo("contato")}
                  style={{
                    padding: "6px 16px", borderRadius: "6px", fontSize: "12px", cursor: "pointer",
                    background: destinoTipo === "contato" ? "rgba(41,171,226,0.15)" : "#1a1a1a",
                    border: `1px solid ${destinoTipo === "contato" ? "#29ABE2" : "#2e2e2e"}`,
                    color: destinoTipo === "contato" ? "#29ABE2" : "#a0a0a0",
                  }}>
                  Contato
                </button>
              </div>
              ...
              {destinoTipo === "grupo" ? (
                <div>
                  <div style={{ display: "flex", gap: "8px" }}>
                    <input className="form-input" placeholder="ID do grupo" value={grupoId} onChange={e => setGrupoId(e.target.value)} style={{ flex: 1 }} />
                    <button className="btn-secondary" onClick={carregarGrupos} disabled={loadingGrupos} style={{ cursor: "pointer", whiteSpace: "nowrap" }}>
                      <RefreshCw size={13} /> {loadingGrupos ? "..." : "Buscar grupos"}
                    </button>
                  </div>
                  {grupos.length > 0 && (
                    <div style={{ marginTop: "8px", display: "flex", flexDirection: "column", gap: "4px", maxHeight: "150px", overflowY: "auto" }}>
                      {grupos.map(g => (
                        <button key={g.id} onClick={() => { setGrupoId(g.id); setGrupoNome(g.subject); setGrupos([]); }}
                          style={{
                          ...
                          <span>{g.subject}</span>
                          <span style={{ color: "#606060" }}>{g.size} membros</span>
                        </button>
                      ))}
                    </div>
                  )}
                  {grupoNome && <p style={{ fontSize: "11px", color: "#29ABE2", marginTop: "4px" }}>Grupo: {grupoNome}</p>}
                </div>
              ) : (
            ...
            <div style={{
              ...
              <p style={{ fontSize: "11px", fontWeight: "600", color: "#a0a0a0", marginBottom: "8px" }}>Variáveis disponíveis (clique para inserir):</p>
            ...
            {showPreview && (
              <div style={{
                background: "#0d1117", border: "1px solid #2e2e2e", borderRadius: "8px",
                padding: "16px", marginBottom: "20px", maxHeight: "300px", overflowY: "auto",
              }}>
                <p style={{ fontSize: "11px", fontWeight: "600", color: "#29ABE2", marginBottom: "8px" }}>Preview do relatório:</p>
                {previewLoading ? (
                  <p style={{ color: "#606060", fontSize: "12px" }}>Gerando preview...</p>
                ) : (
                  <pre style={{ fontSize: "12px", color: "#d0d0d0", whiteSpace: "pre-wrap", fontFamily: "monospace", lineHeight: "1.6", margin: 0 }}>
                    {previewMsg}
                  </pre>
                )}
              </div>
            ...
            <div style={{ display: "flex", gap: "8px", justifyContent: "flex-end" }}>
              ...
              <button className="btn-primary" onClick={salvar} disabled={salvando} style={{ cursor: "pointer" }}>
                <Check size={14} /> {salvando ? "Salvando..." : editando ? "Salvar alterações" : "Criar relatório"}
              </button>
            </div>
          </div>
        </div>
