...
export default function CRMDashboardPage() {
  ...
  const taxaConversao = total > 0 ? ((fechados/total)*100).toFixed(2) : "0.00";
  const pipeline = lf.reduce((a,b)=>a+(b.valor||0),0);
  const perdidos = lf.filter((l)=>l.etapa==="perdido").length;
  // MQLs = leads que:
  // 1. Vieram do Meta (Facebook ou Instagram)
  // 2. Foram criados no período filtrado (iniciaram contato nesse período)
  // 3. Estão em "qualificado" ou passaram por ele (qualificado, reuniao_agendada, proposta_enviada, ganho)
